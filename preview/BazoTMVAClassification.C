...
//Main TMVA function
void BazoTMVAClassification( TString myMethodList = "" )
{
...
//%%PENDING
   ...
   // note that we may also use variable expressions, such as: "3*var1/var2*abs(var3)"
   ...
   */
   ...
   factory->AddSpectator("El",			"Final state primary lepton Energy",	"GeV",	'F');
   factory->AddSpectator("nf",			"Number of final state particles in hadronic shower",	"",	'I');
   ...
   // Automatically download the data file from the server, if it's not already downloaded
	 if (gSystem->AccessPathName( fname ))  // file does not exist in local directory
      gSystem->Exec("curl -O https://raw.githubusercontent.com/sfsanche/bazo_proyecto/master/Data/ultralisk_until7.gst.root");
   
   TFile *input = TFile::Open( fname,"UPDATE" );
   
   std::cout << "--- TMVAClassification       : Using input file: " << input->GetName() << std::endl;
   
   // --- Register the training and test tree. We have both signal and background in the same ttree, identified by the following cuts
   TTree *inputTree		= (TTree*)input->Get("gst");
   TCut signalCut = "nmuons==2";
   TCut backgrCut = "nmuons<2";
   
   // Register the input Tree to the factory
   factory->SetInputTrees    ( inputTree,	signalCut, backgrCut );

   // This is not needed in our case; we leave it here for documentation puroposes 
   ...
   // We don't need this either; our events have all uniform weights.
   ...
   //PENDING: Do we put paper-motivated cuts here?
   // Apply additional cuts on the signal and background samples (can be different)
   TCut mycuts = ""; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
   TCut mycutb = ""; // for example: TCut mycutb = "abs(var1)<0.5";

   // We tell the factory how to use the training and testing events: Separate all the available events by half randomly; renormalization is irrelevant since our events have uniform weights.
   //
   ...
   // ---- Book MVA methods
   //
   // The various method configuration options are available in the corresponding cxx files, eg:
